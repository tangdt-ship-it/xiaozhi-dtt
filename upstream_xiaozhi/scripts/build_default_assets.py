...
def main():
    ...
    if custom_wake_word_config and multinet_model_paths:
        ...
        wake_word_key = custom_wake_word_config['wake_word'].strip().lower()
        display_key = custom_wake_word_config['display'].strip().lower()
        if wake_word_key in ("he lo", "helo", "hello") or display_key in ("he lo", "helo", "hello", "xin chào", "xin chao"):
            seen_commands = {custom_wake_word_config['wake_word'].strip().lower()}
            # DTT: add close English phonetic forms so Vietnamese speakers can say
            # "He lo" naturally while still using the English mn7_en model.
            for alias in ("hello", "he lo", "helo", "he low", "hey lo", "hey low", "halo", "hallo"):
        ...
        multinet_model_info = {
            "language": language,
            "duration": 7000,  # DTT default: English Speech 7s
